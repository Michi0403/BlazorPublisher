param(
    [string]$OutputPath = (Join-Path $PSScriptRoot 'artifacts/source/PublisherStudio-v2.2.6-SOURCE-NOT-COMPILED.zip')
)
...
try {
    ...
    try {
        ...
        foreach ($name in $names) {
            ...
            if ($name -match '(^|/)(\.vs|\.cr|__pycache__|node_modules|artifacts|bin|obj)(/|$)' -or $name -match '\.(?:user|suo|db|pfx|snk|licx|download|pyc)$') { throw "Excluded source-package artifact leaked into the archive: $name" }
        }
        foreach ($required in @(
            ...
            'BlazorPublisher/build/Repair-DocfxNamespacePages.ps1',
            'BlazorPublisher/build/Assert-JavaScriptDiagnostics.ps1',
            'BlazorPublisher/build/javascript-diagnostics-files.sha256',
            ...
            'BlazorPublisher/src/PublisherStudio.Web/PublisherStudio.Web.csproj'
        )) {
