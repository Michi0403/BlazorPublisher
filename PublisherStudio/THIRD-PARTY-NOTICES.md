...
A valid DevExpress license and configured licensed NuGet/npm feeds are required to restore and build these components. Generated DevExpress browser assets and `node_modules` are intentionally not redistributed in the source ZIP. A licensed build copies the required browser files into the published application's local `wwwroot/vendor` directory for offline runtime use.
