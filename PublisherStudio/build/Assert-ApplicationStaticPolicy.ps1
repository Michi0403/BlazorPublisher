...
$files = @(Get-ChildItem -LiteralPath (Join-Path $root 'src\PublisherStudio.Web') -Recurse -File | Where-Object { $_.Extension -in @('.cs','.razor') -and $_.FullName -notmatch '[\\/](?:bin|obj|Migrations|Extensions)[\\/]' -and $_.Name -notlike '*.Designer.cs' -and $_.Name -ne 'Program.cs' })
