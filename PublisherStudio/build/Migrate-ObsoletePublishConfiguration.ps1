...
function Remove-ObsoleteFile([string]$RelativePath) {
...
if ($removed.Count -gt 0) {
    Write-Host "Removed $($removed.Count) obsolete PublisherStudio publish-profile artifact(s) left by an in-place source upgrade:"
    foreach ($relative in $removed | Sort-Object -Unique) { Write-Host "  - $relative" }
}
else {
    Write-Host 'No obsolete PublisherStudio publish-profile artifacts required migration.'
}
