...
$protectedManifestPath = Join-Path $PSScriptRoot 'protected-architecture-files.sha256'
$protectedAssertPath = Join-Path $PSScriptRoot 'Assert-ProtectedArchitectureFiles.ps1'
...
function Get-MaintainedJavaScriptFiles {
    $jsRoot = Join-Path $root 'src\PublisherStudio.Web\wwwroot\js'
    return @(
        Get-ChildItem -LiteralPath $jsRoot -File -Filter '*.js' |
...
$expectedProtected = @(Get-ExpectedProtectedFiles $protectedAssertPath | Where-Object { $_ -ne 'build/protected-architecture-files.sha256' })
...
Invoke-RequiredSafeguard 'build/Assert-PublishConfiguration.ps1'
...
try {
    ...
    Invoke-RequiredSafeguard 'build/Assert-ProtectedArchitectureFiles.ps1'
}
catch {
