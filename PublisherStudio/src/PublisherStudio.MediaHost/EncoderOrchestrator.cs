...
public sealed class EncoderOrchestrator(ILogger<EncoderOrchestrator> logger)
{
    ...
    {
        session.Encoder ??= new EncoderSessionController(session, _logger);
...
public sealed class EncoderSessionController : IDisposable
{
    ...
    public EncoderSessionController(MediaSession session, ILogger logger)
    {
        _session = session;
        _logger = logger;
    ...
    private void StartProcess(string key, Guid? inputId, IReadOnlyList<string> arguments, bool restart = false)
    {
        ...
        var executable = string.IsNullOrWhiteSpace(_session.FfmpegPath) ? "ffmpeg" : _session.FfmpegPath;
...
internal static class FfmpegEncoderResolver
{
    ...
    {
        var executable = string.IsNullOrWhiteSpace(configuredPath) ? "ffmpeg" : configuredPath;
