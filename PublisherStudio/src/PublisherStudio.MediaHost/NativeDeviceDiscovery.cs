...
public sealed record NativeMediaDeviceInfo(string Id, string Name, string Kind, string Backend, string? ProcessId = null, string? WindowTitle = null);

public static class NativeDeviceDiscovery
{
    public static async Task<IReadOnlyList<NativeMediaDeviceInfo>> DiscoverAsync(string? ffmpegPath, CancellationToken cancellationToken)
    {
        var result = new List<NativeMediaDeviceInfo>();
        ...
        else if (OperatingSystem.IsLinux())
        {
            foreach (var path in Directory.Exists("/dev") ? Directory.EnumerateFiles("/dev", "video*") : [])
                result.Add(new NativeMediaDeviceInfo(path, Path.GetFileName(path), "CaptureDevice", "v4l2"));
        }
        return result
    ...
    private static async Task<IReadOnlyList<NativeMediaDeviceInfo>> DiscoverDirectShowAsync(string? ffmpegPath, CancellationToken cancellationToken)
    {
        ...
        foreach (var raw in output.Split('\n'))
        {
            ...
            result.Add(new NativeMediaDeviceInfo(name, name, kind, "dshow"));
        }
        return result;
    }

    private static async Task<IReadOnlyList<NativeMediaDeviceInfo>> DiscoverAvFoundationAsync(string? ffmpegPath, CancellationToken cancellationToken)
    {
        ...
        foreach (var raw in output.Split('\n'))
        {
            ...
            if (kind.Length == 0 || !match.Success) continue;
            result.Add(new NativeMediaDeviceInfo(match.Groups["index"].Value, match.Groups["name"].Value.Trim(), kind, "avfoundation"));
        }
        return result;
    }

    private static IReadOnlyList<NativeMediaDeviceInfo> DiscoverWindowsProcesses()
    {
        var result = new List<NativeMediaDeviceInfo>();
        foreach (var process in Process.GetProcesses())
        {
            try
            {
                ...
                result.Add(new NativeMediaDeviceInfo(
    ...
    {
        var executable = string.IsNullOrWhiteSpace(ffmpegPath) ? "ffmpeg" : ffmpegPath;
        try
        {
