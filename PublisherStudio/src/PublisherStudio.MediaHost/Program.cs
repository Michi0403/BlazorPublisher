...
builder.Services.AddHostedService<GlobalHotkeyService>(provider => provider.GetRequiredService<GlobalHotkeyService>());
builder.Services.AddSingleton<EncoderOrchestrator>();
builder.Services.AddSingleton<NativeCaptureRegistry>();
...
app.MapGet("/api/mediahost/capabilities", () => Results.Ok(new
{
    ...
    note = "The host owns encoder orchestration, recording, LAN delivery, native capture-card/device discovery and Windows global hotkeys. Windows process-tree audio loopback is built in on Windows 10 build 20348 or later; browser window-audio remains the cross-platform fallback."
}));
...
Console.WriteLine($"PublisherStudio Media Host listening on http://127.0.0.1:{port}");
await app.RunAsync();

static int ResolvePort(string[] args)
{
    for (var index = 0; index < args.Length - 1; index++)
        if (string.Equals(args[index], "--port", StringComparison.OrdinalIgnoreCase) && int.TryParse(args[index + 1], out var value))
            return Math.Clamp(value, 1024, 65535);
    return int.TryParse(Environment.GetEnvironmentVariable("PUBLISHERSTUDIO_MEDIAHOST_PORT"), out var configured)
        ? Math.Clamp(configured, 1024, 65535)
        : 17847;
}
