...
public sealed partial class VideoProjectImportService
{
    ...
    private static VideoProjectImportResult ImportOtio(
        byte[] bytes,
        string fileName,
    ...
    private static void ImportOtioNestedStack(
        JsonElement stack,
    ...
    private static void ImportOtioTrackChildren(
    ...
    private static double OtioLinearSpeed(JsonElement clip, List<InterchangeIssue> issues)
    {
    ...
    private static void ImportOtioMarkers(
        JsonElement owner,
    ...
    private static VideoProjectImportResult ImportMlt(byte[] bytes, string fileName)
    {
    ...
    private static void ImportMltPlaylist(
        XElement playlist,
        MediaTimelineTrack track,
    ...
    private static void ImportMltTransitions(
        XElement tractor,
        VideoProjectDocument project,
        List<InterchangeIssue> issues,
        double frameRate)
    ...
    private static VideoProjectImportResult ImportXges(byte[] bytes, string fileName)
    {
        var document = ParseSafeXml(bytes);
    ...
    private static PublicationMediaSegment XgesSegment(
        string name,
        string assetId,
        XElement? asset,
        double start,
    ...
    private static VideoProjectImportResult ImportOpenShot(byte[] bytes, string fileName)
    {
        using var document = JsonDocument.Parse(bytes, new JsonDocumentOptions
        {
            AllowTrailingCommas = true,
    ...
    private static VideoProjectImportResult ImportEdl(byte[] bytes, string fileName)
    {
        ...
        foreach (var rawLine in text.Replace("\r", string.Empty).Split('\n'))
        {
            ...
            var match = EdlEventRegex().Match(line);
            if (match.Success)
            {
            ...
            var clipName = EdlClipNameRegex().Match(line);
            if (clipName.Success)
            {
            ...
            var sourceFile = EdlSourceFileRegex().Match(line);
    ...
    private static VideoProjectImportResult FinalizeResult(VideoProjectImportResult result, string fileName)
    {
    ...
    private static XDocument ParseSafeXml(byte[] bytes)
    {
    ...
    private static string NormalizeOtioTarget(string targetUrl)
    {
    ...
    private static IEnumerable<JsonElement> JsonArray(JsonElement owner, string property)
    {
    ...
    private static string JsonString(JsonElement owner, string property, string fallback = "")
    {
    ...
    private static double JsonDouble(JsonElement owner, string property, double fallback = 0)
    {
        if (owner.ValueKind != JsonValueKind.Object || !owner.TryGetProperty(property, out var value)) return fallback;
    ...
    private static bool JsonBool(JsonElement owner, string property, bool fallback)
    {
        if (owner.ValueKind != JsonValueKind.Object || !owner.TryGetProperty(property, out var value)) return fallback;
    ...
    private static double FindOtioRate(JsonElement root, double fallback)
    {
    ...
    private static double OtioTimeRangeStart(JsonElement owner, double fallbackRate)
    {
        var range = owner;
    ...
    private static double OtioTimeRangeDuration(JsonElement owner, double fallbackRate)
    {
        var range = owner;
        if (owner.ValueKind == JsonValueKind.Object && owner.TryGetProperty("source_range", out var sourceRange)) range = sourceRange;
    ...
    private static double OtioRationalSeconds(JsonElement owner, string property, double fallbackRate)
    {
    ...
    private static double OtioSourceRate(JsonElement owner, double fallbackRate)
    {
        if (owner.ValueKind == JsonValueKind.Object
    ...
    private static string OtioMarkerColor(string color) => color.ToLowerInvariant() switch
    {
        "red" => "#ef4444",
        "orange" => "#f97316",
        "yellow" => "#eab308",
    ...
    private static double AttributeDouble(XElement? element, string name, double fallback)
        => ParseDouble(Attribute(element, name), fallback);

    private static string Property(XElement? element, string name, string fallback = "")
    {
    ...
    private static double PropertyDouble(XElement? element, string name, double fallback)
        => ParseDouble(Property(element, name), fallback);

    private static double Ratio(double numerator, double denominator, double fallback)
        => denominator > 0 && numerator > 0 ? numerator / denominator : fallback;

    private static double MltPosition(string value, double frameRate)
    {
    ...
    private static double MltDuration(string value, double frameRate)
    {
        if (string.IsNullOrWhiteSpace(value)) return 0;
        if (value.Contains(':')) return ParseClock(value, frameRate);
    ...
    private static double ParseMltSpeed(string resource, XElement? producer)
    {
    ...
    private static double ParseCapsFrameRate(string serialized, double fallback)
    {
    ...
    private static string PropertyFromSerialized(string serialized, string property, string fallback)
    {
    ...
    private static double Nanoseconds(double value) => value / 1_000_000_000d;

    private static double OpenShotFrameRate(JsonElement root)
    {
    ...
    private static int OpenShotProfileInt(JsonElement root, string property, int fallback)
    {
        if (root.TryGetProperty("profile", out var profile))
    ...
    private static double ParseEdlRate(string text, double fallback)
    {
    ...
    private static string EdlTitle(string text, string fallback)
    {
        var match = Regex.Match(text, @"(?im)^\s*TITLE\s*:\s*(?<title>.+?)\s*$");
        return match.Success ? match.Groups["title"].Value.Trim() : fallback;
    }

    private static double ParseTimecode(string value, double frameRate)
    {
    ...
    private static double ParseClock(string value, double frameRate)
    {
    ...
    private static VideoEffectLayer DefaultVideoLayer(string name, double start, double end) => new()
    {
    ...
    [GeneratedRegex(@"^\s*(?<event>\d+)\s+(?<reel>\S+)\s+(?<track>\S+)\s+(?<transition>\S+)(?:\s+\d+)?\s+(?<sourceIn>\d{2}[:;]\d{2}[:;]\d{2}[:;]\d{2})\s+(?<sourceOut>\d{2}[:;]\d{2}[:;]\d{2}[:;]\d{2})\s+(?<recordIn>\d{2}[:;]\d{2}[:;]\d{2}[:;]\d{2})\s+(?<recordOut>\d{2}[:;]\d{2}[:;]\d{2}[:;]\d{2})\s*$", RegexOptions.IgnoreCase)]
    private static partial Regex EdlEventRegex();

    [GeneratedRegex(@"^\s*\*\s*(?:FROM\s+)?CLIP\s+NAME\s*:\s*(?<name>.+?)\s*$", RegexOptions.IgnoreCase)]
    private static partial Regex EdlClipNameRegex();

    [GeneratedRegex(@"^\s*\*\s*(?:SOURCE|FROM)\s+FILE\s*:\s*(?<path>.+?)\s*$", RegexOptions.IgnoreCase)]
    private static partial Regex EdlSourceFileRegex();
}
