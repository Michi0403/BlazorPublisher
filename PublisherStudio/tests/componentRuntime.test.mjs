...
assert.match(publicationModel, /FormatVersion \{ get; set; \} = "1\.48"/);
...
assert.match(view, /Item\.MenuSourceMode == PublicationMenuSourceMode\.DataConnection/);
assert.match(view, /private bool RequiresDataObject/);
...
assert.match(runtime, /normalizeDateRows\(config/);
...
assert.match(exporter, /Commit the selection on pointerup/);
assert.match(exporter, /<script>\$\{safeScript\(componentRuntimeSource\)\}<\/script>/);
assert.match(publicationModel, /PublicationConnectorPort/);
assert.match(publicationModel, /Guid\? PortId/);
assert.match(publicationModel, /Control1X/);
assert.match(pageSurface, /CommitConnectorAdvanced/);
assert.match(pageSurface, /ReconnectConnectorAdvanced/);
assert.match(pageSurface, /CommitConnectorRoute/);
assert.match(exporter, /component-pending/);
assert.match(exporter, /connector-control/);
assert.match(exporter, /createConnectorPortPreview/);
assert.match(exporter, /export function initializeSignalConnectors/);
