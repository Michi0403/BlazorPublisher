...
assert.match(installerProject, /<None Update="Install\.cmd">[\s\S]*?<CopyToPublishDirectory>Always<\/CopyToPublishDirectory>/);
assert.match(installerProject, /<None Update="Update\.cmd">[\s\S]*?<CopyToPublishDirectory>Always<\/CopyToPublishDirectory>/);
assert.match(installerProject, /<None Update="Start\.cmd">[\s\S]*?<CopyToPublishDirectory>Always<\/CopyToPublishDirectory>/);
assert.match(installerProject, /<None Update="Uninstall\.cmd">[\s\S]*?<CopyToPublishDirectory>Always<\/CopyToPublishDirectory>/);
assert.match(installLauncher, /--install-blazorpublisher --force-delete --start-blazorpublisher --port 58071 --shortcuts/);
assert.match(updateLauncher, /--update-blazorpublisher --start-blazorpublisher --port 58071 --shortcuts/);
...
assert.match(host, /DefaultPort\s*=\s*58071/);
