...
const model = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'BusinessObjects', 'PublicationStreamingModels.cs'), 'utf8');
...
const sessionService = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'UseCases', 'Editor', 'StreamingSessionService.cs'), 'utf8');
const streamingControllersDirectory = path.join(root, 'src', 'PublisherStudio.Web', 'Controllers', 'Streaming', 'UseCases');
const streamingControllers = fs.readdirSync(streamingControllersDirectory).filter(name => name.endsWith('.cs')).map(name => fs.readFileSync(path.join(streamingControllersDirectory, name), 'utf8')).join('\n');
const sessionRegistry = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'Sessions', 'MediaSessionRegistry.cs'), 'utf8');
const mediaSession = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'Sessions', 'MediaSession.cs'), 'utf8');
const hotkeys = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'Hotkeys', 'GlobalHotkeyService.cs'), 'utf8');
const nowPlaying = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'Metadata', 'NowPlayingReader.cs'), 'utf8');
const streamingComposition = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'StreamingServiceCollectionExtensions.cs'), 'utf8');
const runtimeUseCases = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'UseCases', 'Runtime', 'StreamingRuntimeUseCases.cs'), 'utf8');
const mediaHost = [streamingControllers, sessionRegistry, mediaSession, hotkeys, nowPlaying, streamingComposition, runtimeUseCases].join('\n');
const encoder = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'Encoding', 'EncoderOrchestrator.cs'), 'utf8');
const lanServer = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'Lan', 'LanStreamingServer.cs'), 'utf8');
const nativeCapture = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'Capture', 'NativeCaptureRegistry.cs'), 'utf8');
const nativeDevices = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'Capture', 'NativeDeviceDiscovery.cs'), 'utf8');
const platformChat = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'Chat', 'PlatformChatService.cs'), 'utf8');
const rtspServer = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'Lan', 'RtspLanServer.cs'), 'utf8');
const webRtcHub = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'Lan', 'WebRtcSignalingService.cs'), 'utf8');
const platformChatHub = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Hubs', 'Streaming', 'Chat', 'PlatformChatHub.cs'), 'utf8');
...
const profileStore = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'Profiles', 'StreamingProfileStore.cs'), 'utf8');
const twitchOAuth = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'OAuth', 'TwitchOAuthService.cs'), 'utf8');
const twitchMaintenance = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'HostedServices', 'Streaming', 'TwitchOAuthMaintenanceService.cs'), 'utf8');
...
const streamingClient = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'MediaHost', 'StreamingMediaHostClient.cs'), 'utf8');
const ffmpegLocator = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.Web', 'Services', 'Streaming', 'Encoding', 'FfmpegLocator.cs'), 'utf8');
...
assert.match(streamingControllers, /api\/mediahost\/sessions\/\{sessionId:guid\}\/chat/);
assert.match(streamingControllers, /HttpGet\("\{outputId:guid\}\/websocket"\)/);
assert.match(streamingControllers, /HttpPost\("\{outputId:guid\}\/send"\)/);
assert.match(streamingControllers, /PlatformChatHub/);
assert.match(platformChatHub, /namespace PublisherStudio\.Hubs\.Streaming\.Chat/);
assert.match(webRtcEntryHub, /namespace PublisherStudio\.Hubs\.Streaming\.Lan/);
...
assert.equal((platformChat.match(/public async ValueTask DisposeAsync\(\)/g) || []).length, 3, 'The platform Chat service and both provider adapters must expose one disposal method.');
...
assert.match(streamingClient, /Task<List<PublisherStudio\.BusinessObjects\.NativeMediaDeviceInfo>>/);
assert.match(runtimeUseCases, /new PublisherStudio\.BusinessObjects\.NativeMediaDeviceInfo/);
...
assert.match(runtimeUseCases, /Version = "2\.0\.4"/);
...
assert.match(twitchMaintenance, /TimeSpan\.FromHours\(1\)/);
