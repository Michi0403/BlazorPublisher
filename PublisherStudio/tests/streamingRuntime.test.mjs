...
const mediaHost = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.MediaHost', 'Program.cs'), 'utf8');
const encoder = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.MediaHost', 'EncoderOrchestrator.cs'), 'utf8');
const lanServer = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.MediaHost', 'LanStreamingServer.cs'), 'utf8');
const nativeCapture = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.MediaHost', 'NativeCaptureRegistry.cs'), 'utf8');
const nativeDevices = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.MediaHost', 'NativeDeviceDiscovery.cs'), 'utf8');
const platformChat = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.MediaHost', 'PlatformChatHub.cs'), 'utf8');
const rtspServer = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.MediaHost', 'RtspLanServer.cs'), 'utf8');
const webRtcHub = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.MediaHost', 'WebRtcSignalingHub.cs'), 'utf8');
const processLoopback = fs.readFileSync(path.join(root, 'src', 'PublisherStudio.MediaHost', 'WindowsProcessLoopbackCapture.cs'), 'utf8');
...
assert.match(studio, /Global hotkeys are registered by the Windows Media Host/);
...
await Promise.resolve();
assert.equal(calls.length, 1, 'Global hotkeys must be owned by the Media Host, not double-fired by the browser.');
