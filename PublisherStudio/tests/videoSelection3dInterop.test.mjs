...
const polygonGeometry = read('src', 'PublisherStudio.Web', 'Services', 'VideoStudio', 'Export', 'PolygonGeometryService.cs');
const openScadAdapter = read('src', 'PublisherStudio.Web', 'Services', 'OpenScad', 'OpenScadVideoLayerAdapter.cs');
const browserRuntime = read('src', 'PublisherStudio.Web', 'Services', 'VideoStudio', 'Export', 'BrowserRuntimeTemplateService.cs');
...
assert.match(interchange, /interface IVideoLayerInterchangeService/);
assert.match(interchange, /IPolygonGeometryService geometry/);
assert.match(interchange, /IOpenScadVideoLayerAdapter openScad/);
assert.match(interchange, /IBrowserRuntimeTemplateService browserRuntime/);
assert.match(interchange, /CreateDefaultBlobLayer/);
assert.match(interchange, /HTML canvas · OpenSCAD interchange/);
assert.doesNotMatch(interchange, /private static/);
assert.doesNotMatch(interchange, /Math\.Hypot/);
assert.match(polygonGeometry, /interface IPolygonGeometryService/);
assert.match(polygonGeometry, /double Distance\(double deltaX, double deltaY\)/);
assert.doesNotMatch(polygonGeometry, /Math\.Hypot/);
assert.match(openScadAdapter, /function ps_blob_points/);
assert.match(openScadAdapter, /minkowski\(\)/);
assert.match(openScadAdapter, /linear_extrude/);
assert.match(openScadAdapter, /polygon\(points=pts\)/);
assert.match(openScadAdapter, /\$t animates/);
assert.match(browserRuntime, /requestAnimationFrame\(draw\)/);
