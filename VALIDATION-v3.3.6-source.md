# PublisherStudio 3.3.6 source validation

Static validation covers version consistency, the synchronized `LocalGPT.ReleasePackaging` 1.0.2 PDFsharp source fallback, adaptive memory-based browser PDF chunking, preservation of every rendered chunk until the managed merge completes, optional qpdf/Ghostscript optimization, the compressed embedded documentation PDF contract, Full/self-contained Unix/macOS packaging, resumable notarization/reuse, Homebrew RPM support, architecture/cross-platform boundaries, async/service resilience, and the four reviewed InteractiveServer boundaries.
