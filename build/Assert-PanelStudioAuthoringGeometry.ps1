...
Require $panelStudio 'PanelDesignWidthPxValue[\s\S]{0,180}_draft\?\.CanvasWidth' 'Panel Studio design width must come from the panel-local CanvasWidth, not Mainframe placement Width.'
Require $panelStudio 'PanelDesignHeightPxValue[\s\S]{0,180}_draft\?\.CanvasHeight' 'Panel Studio design height must come from the panel-local CanvasHeight, not Mainframe placement Height.'
...
Require $interop 'Math\.min\(1, availableWidth / width, availableHeight / height\)' 'Panel Studio may shrink an oversized panel but must not upscale it to fill the Mainframe-like workspace.'
