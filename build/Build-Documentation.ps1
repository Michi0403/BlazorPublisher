...
$documentationToolCacheRoot = if (-not [string]::IsNullOrWhiteSpace($DocumentationCacheRoot)) {
    ...
    Get-PublisherStudioDocumentationToolCacheRoot -FallbackRoot $fallbackToolRoot
}
...
function New-PublisherStudioHtmlPrintBook {
    ...
    $allPages = @(Get-PublisherStudioPrintPageFiles -SiteRoot $SiteRoot)
    ...
    for ($index = 0; $index -lt $pages.Count; $index++) {
        ...
        $anchor = 'publisherstudio-document-{0:D5}' -f (($StartIndex + $index) + 1)
            ...
            try {
                ...
                while (-not $process.HasExited -and [DateTime]::UtcNow -lt $processDeadline) {
                    ...
                    if ($null -ne $livePdfFile -and $livePdfFile.Length -ge $MinimumBytes) {
                        ...
                        if ($liveStableLengthChecks -ge 4 -and (Test-PublisherStudioCompletePdf -Path $PdfPath -MinimumBytes $MinimumBytes)) {
                ...
                if (Test-PublisherStudioCompletePdf -Path $PdfPath -MinimumBytes $MinimumBytes) {
                    if ($lastExitCode -ne 0) {
                        $diagnostics.Add("The browser returned exit code $lastExitCode after writing a valid PDF candidate; PublisherStudio accepted the validated file.")
                    }
                    return [pscustomobject]@{
                        Succeeded = $true
...
function Invoke-PublisherStudioChunkedBrowserPdf {
    param(
        [Parameter(Mandatory)][string]$BrowserPath,
        [Parameter(Mandatory)][string]$SiteRoot,
        [Parameter(Mandatory)][string]$PdfPath,
    ...
    Remove-PublisherStudioTemporaryPath -Path $chunkRoot
    ...
    try {
        ...
        $indexedPages = New-PublisherStudioHtmlPrintBook -SiteRoot $SiteRoot -DestinationPath $frontMatterHtml -FrontMatterOnly
        if (Test-PublisherStudioCompletePdf -Path $frontMatterPdf -MinimumBytes 65536) {
            $reusedChunks++
        ...
        for ($start = 0; $start -lt $TotalPageCount; $start += $ChunkPages) {
            ...
            $writtenPages = New-PublisherStudioHtmlPrintBook -SiteRoot $SiteRoot -DestinationPath $htmlPath -StartIndex $start -MaximumPages $ChunkPages -SuppressFrontMatter
            if (Test-PublisherStudioCompletePdf -Path $chunkPdf -MinimumBytes 65536) {
                $reusedChunks++
        ...
        if ($mergeExitCode -ne 0 -or -not (Test-PublisherStudioCompletePdf -Path $PdfPath -MinimumBytes $MinimumBytes)) {
    ...
    finally { Remove-PublisherStudioTemporaryPath -Path $chunkRoot }
}

function Resolve-PublisherStudioHomebrew {
...
function Find-PublisherStudioQpdf {
    $candidates = [System.Collections.Generic.List[string]]::new()
    ...
    if ([Runtime.InteropServices.RuntimeInformation]::IsOSPlatform([Runtime.InteropServices.OSPlatform]::OSX)) {
        ...
        $brewPath = Resolve-PublisherStudioHomebrew
...
function Ensure-PublisherStudioQpdf {
    $existing = Find-PublisherStudioQpdf
    ...
    return Find-PublisherStudioQpdf
}
...
function Ensure-PublisherStudioGhostscript {
    $existing = Find-PublisherStudioGhostscript
    if (-not [string]::IsNullOrWhiteSpace($existing)) { return $existing }
    if ($DisablePdfToolProvisioning) { return $null }
    if (-not [Runtime.InteropServices.RuntimeInformation]::IsOSPlatform([Runtime.InteropServices.OSPlatform]::OSX)) { return $null }
    ...
    if ([string]::IsNullOrWhiteSpace($brewPath)) {
        Write-Warning "Homebrew was not found. Automatic Ghostscript provisioning cannot continue. Install Homebrew once, install Ghostscript manually, or set PUBLISHERSTUDIO_DOCUMENTATION_PDF_COMPRESSOR to a compatible gs executable."
        return $null
    }
    ...
    $installed = Find-PublisherStudioGhostscript
...
function Optimize-PublisherStudioPdf {
    ...
    if (-not [string]::IsNullOrWhiteSpace($PackagingTool)) {
        $qpdf = Ensure-PublisherStudioQpdf
        $existingGhostscript = Find-PublisherStudioGhostscript
        ...
        if ($managedExitCode -eq 0 -and $null -ne $managedOptimized -and $managedOptimized.Length -lt $source.Length -and (Test-PublisherStudioCompletePdf -Path $managedTemporary -MinimumBytes $MinimumBytes)) {
    ...
    $ghostscript = Ensure-PublisherStudioGhostscript
...
function Ensure-PublisherStudioDocfxToolForPdfFallback {
    ...
    # the command lazily here so Invoke-PublisherStudioDocfx never receives a null command target.
            ...
            try {
                ...
                if ($null -ne $browser) {
                    if ($requiresChunkedBrowserPdf) {
                        ...
                        $browserResult = Invoke-PublisherStudioChunkedBrowserPdf -BrowserPath $browser.Path -SiteRoot $siteRoot -PdfPath $pdfPath -WorkingRoot $printBookRoot -PackagingTool $PackagingTool -TotalPageCount $pdfSourcePageCount -ChunkPages $browserPdfChunkPages -ChunkCacheRoot $browserChunkCacheRoot -MinimumBytes $minimumCompletePdfBytes
                    ...
                    elseif ($pdfSourcePageCount -le $maximumBrowserPrintSourcePages) {
                        ...
                        $browserResult = Invoke-PublisherStudioBrowserPdf -BrowserPath $browser.Path -HtmlPath $printBookPath -PdfPath $pdfPath -WorkingRoot $printBookRoot -MinimumBytes $minimumCompletePdfBytes
                        $pdfCandidateCount = if (Test-Path -LiteralPath $pdfPath -PathType Leaf) { 1 } else { 0 }
                        if ($browserResult.Succeeded) {
                            $pdfGenerated = Test-PublisherStudioCompletePdf -Path $pdfPath -MinimumBytes $minimumCompletePdfBytes
                            if ($pdfGenerated) {
        ...
        if (-not $pdfGenerated) {
            ...
            try {
                ...
                if ($null -ne $nodeInfo) {
                    ...
                    Write-Host "Generating the complete PDF with the DocFX PDF plug-in and Node.js $nodeVersionUsed. This can take several minutes for $pdfSourcePageCount pages; DocFX output is streamed live below." -ForegroundColor Cyan
...
if (-not $pdfGenerated) {
    ...
    foreach ($htmlFile in Get-ChildItem -LiteralPath $siteRoot -Filter "*.html" -File -Recurse -ErrorAction SilentlyContinue) {
        ...
        $updatedHtmlText = [regex]::Replace(
            ...
            'href="#" aria-disabled="true" data-publisherstudio-pdf-unavailable="true" title="PDF handbook is unavailable only in this explicitly HTML-only diagnostic documentation build"')
...
if ($RequirePdf -or $pdfGenerated) {
    ...
    if (-not (Test-Path -LiteralPath $sourcePdfPath -PathType Leaf)) {
        throw "Embedded PublisherStudio documentation PDF was not published into the runtime help-docs tree: $sourcePdfPath"
    }
    $sourcePdfBytes = [long](Get-Item -LiteralPath $sourcePdfPath).Length
    if ($sourcePdfBytes -gt $maximumSanePdfBytes) {
        throw "Embedded PublisherStudio documentation PDF is $sourcePdfBytes bytes, above the configured sane-size ceiling of $maximumSanePdfBytes bytes."
    }
