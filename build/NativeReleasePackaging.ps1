...
function Get-MacFileInventory([string]$AppPath) {
    ...
    # Keep bundle inspection batched. Invoking `file` once per payload file can make PowerShell on
    # macOS emit one MallocStackLogging diagnostic per child launch, drowning the useful release
    # output and multiplying process startup overhead. Brief mode preserves one description per
    # input path, so the existing architecture checks remain exact without the subprocess storm.
...
SHOW_CONSOLE="${PUBLISHERSTUDIO_SHOW_CONSOLE:-1}"
...
ensure_native_launcher_process() {
  ...
  if [ "$hardware" = "arm64" ] && [ "$translated" = "1" ] && [ "${PUBLISHERSTUDIO_NATIVE_REEXEC:-0}" != "1" ] && [ -x /usr/bin/arch ]; then
    ...
    export PUBLISHERSTUDIO_NATIVE_REEXEC=1
...
verify_bundle_identity() {
  ...
  case "$runtime_source" in ''|*[!0-9a-fA-F]*) show_failure "The packaged source fingerprint is invalid: $RUNTIME_SOURCE_FILE"; return 1 ;; esac
...
terminate_stale_processes() {
  ...
  for stale_pid in $stale_pids; do
    [ "$stale_pid" = "$$" ] && continue
    if /bin/kill -0 "$stale_pid" 2>/dev/null; then /bin/kill -KILL "$stale_pid" 2>/dev/null || true; fi
  done
}
...
printf '%s\n' "$(date '+%Y-%m-%d %H:%M:%S') Starting $PRODUCT from $BIN with workingDirectory=$(pwd -P) and applicationLog=$APP_LOG_FILE." >>"$LOG_FILE"
"$BIN" >>"$LOG_FILE" 2>&1 &
pid=$!
...
stop_running_product() {
  ...
  while [ $i -lt 20 ]; do
    remaining=""
    for pid in $pids; do if /bin/kill -0 "$pid" 2>/dev/null; then remaining="$remaining $pid"; fi; done
    [ -z "$remaining" ] && return 0
    /bin/sleep 0.25
    i=$((i+1))
  done
  for pid in $pids; do if /bin/kill -0 "$pid" 2>/dev/null; then /bin/kill -KILL "$pid" 2>/dev/null || true; fi; done
}
'@
...
if [ -f "$MARKER" ]; then
  ...
  if [ -n "${console_uid:-}" ] && [ -x /bin/launchctl ]; then /bin/launchctl asuser "$console_uid" /usr/bin/sudo -u "$console_user" /usr/bin/open "$APP_PATH" >/dev/null 2>&1 || true; fi
fi
exit 0
'@
