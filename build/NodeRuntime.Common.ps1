...
function Get-PublisherStudioNodeHostDescriptor {
    ...
    switch -Regex ($architecture) {
        ...
        default { throw "Unsupported host architecture '$architecture' for the PublisherStudio Node.js bootstrap." }
    }
...
function Get-PublisherStudioDocumentationToolCacheRoot {
    param(
    ...
    if (-not [string]::IsNullOrWhiteSpace($homePath)) {
        return Join-Path $homePath '.local/share/PublisherStudio/DocumentationTools'
    }
    ...
    throw 'Unable to determine a writable per-user cache directory for PublisherStudio documentation tools.'
}

function Get-PublisherStudioNodeDistributionDescriptor {
    param(
        [Parameter(Mandatory)][string]$Version
    )

    $hostInfo = Get-PublisherStudioNodeHostDescriptor
    ...
    switch ($hostInfo.Platform) {
        ...
        default {
            throw "Unsupported host platform '$($hostInfo.Platform)' for the PublisherStudio Node.js bootstrap."
        }
    }
...
function Set-PublisherStudioNodeProcessEnvironment {
    param(
...
function Find-PublisherStudioNodeRuntime {
    param(
    ...
    $distribution = Get-PublisherStudioNodeDistributionDescriptor -Version $Version
    ...
    foreach ($candidate in $candidates) {
        ...
        $nodeInfo = Get-PublisherStudioNodeInfo -Path $fullPath -MinimumMajor $MinimumMajor -Provisioned $isProvisioned -Platform $distribution.Platform -Architecture $distribution.Architecture
...
function Get-PublisherStudioNodeArchiveHash {
    param(
...
function Install-PublisherStudioNodeRuntime {
    param(
        [Parameter(Mandatory)][string]$CacheRoot,
        [Parameter(Mandatory)][string]$Version,
        [int]$MinimumMajor = 20
    ...
    $provisionedRoot = Join-Path $CacheRoot $distribution.RootName
    $provisionedExecutable = Join-Path $provisionedRoot $distribution.ExecutableRelativePath
    $existing = Get-PublisherStudioNodeInfo -Path $provisionedExecutable -MinimumMajor $MinimumMajor -Provisioned $true -Platform $distribution.Platform -Architecture $distribution.Architecture
    if ($null -ne $existing) {
        Set-PublisherStudioNodeProcessEnvironment -NodeInfo $existing
        return $existing
    }
    ...
    $expectedHash = Get-PublisherStudioNodeArchiveHash -ManifestPath $manifestPath -ArchiveName $distribution.ArchiveName
    ...
    if (-not (Test-Path -LiteralPath $archivePath -PathType Leaf)) {
        Write-Host "Node.js $MinimumMajor+ was not found. Downloading verified Node.js v$Version ($($distribution.Platform)-$($distribution.Architecture)) for PublisherStudio documentation tooling..." -ForegroundColor Cyan
        ...
        try {
            ...
            Remove-Item -LiteralPath $archiveDownloadPath -Force -ErrorAction SilentlyContinue
            throw "Node.js v$Version could not be provisioned for PublisherStudio documentation tooling: $($_.Exception.Message)"
        }
    }
    ...
    $installed = Get-PublisherStudioNodeInfo -Path $provisionedExecutable -MinimumMajor $MinimumMajor -Provisioned $true -Platform $distribution.Platform -Architecture $distribution.Architecture
    if ($null -eq $installed) {
        throw "The provisioned Node.js runtime could not be executed: $provisionedExecutable"
    }

    Set-PublisherStudioNodeProcessEnvironment -NodeInfo $installed
    return $installed
}

function Resolve-PublisherStudioNodeRuntime {
    param(
        [Parameter(Mandatory)][string]$CacheRoot,
        [Parameter(Mandatory)][string]$Version,
        [int]$MinimumMajor = 20,
    ...
    $nodeInfo = Find-PublisherStudioNodeRuntime -CacheRoot $CacheRoot -Version $Version -MinimumMajor $MinimumMajor
    if ($PreferCompatibleLts) {
        if ($null -ne $nodeInfo -and $nodeInfo.Major -le $MaximumPreferredMajor) {
            Set-PublisherStudioNodeProcessEnvironment -NodeInfo $nodeInfo
            return $nodeInfo
        }

        if ($AllowProvisioning) {
            try {
                return Install-PublisherStudioNodeRuntime -CacheRoot $CacheRoot -Version $Version -MinimumMajor $MinimumMajor
            }
            catch {
    ...
    if ($null -eq $nodeInfo -and $AllowProvisioning) {
        $nodeInfo = Install-PublisherStudioNodeRuntime -CacheRoot $CacheRoot -Version $Version -MinimumMajor $MinimumMajor
    }
