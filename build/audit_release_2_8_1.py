#!/usr/bin/env python3
"""Static release audit for PublisherStudio 2.8.1 adaptive media quality plus XML-documentation build repair."""
...
try:
    ...
    require(
        ...
        "private (int Width, int Height) FitWithin",
        "private double Positive",
    )

    import subprocess
    result = subprocess.run(
        [sys.executable, str(root / "build/Assert-XmlDocumentationCoverage.py"), str(root / "src")],
        cwd=root / "build", text=True, capture_output=True)
    if result.returncode != 0:
        raise AssertionError("XML documentation validator failed: " + result.stdout + result.stderr)
    checks += 1

    print(f"PublisherStudio 2.8.1 adaptive-media/XML-doc source audit passed: {checks} checks.")
except (AssertionError, KeyError, json.JSONDecodeError) as exc:
    print(f"PublisherStudio 2.8.1 adaptive-media/XML-doc source audit failed: {exc}", file=sys.stderr)
    sys.exit(1)
