#!/usr/bin/env python3
"""Static source audit for PublisherStudio 2.8.8 strict LocalGPT architecture maintenance."""
...
try:
 for rel in ['src/PublisherStudio.Web/PublisherStudio.Web.csproj','src/PublisherStudio.InstallerConsole/PublisherStudio.InstallerConsole.csproj']:
  req(rel,'<Version>2.8.8</Version>')
 req('Directory.Build.props','<LocalGptWireProtocolVersion>2.1.1</LocalGptWireProtocolVersion>')
 req('src/PublisherStudio.Web/Components/App.razor','css/site.css?v=20260818-288','videoEffectRuntime.js?v=2.8.8','publisherInterop.js?v=2.8.8')
 for rel in ['src/PublisherStudio.Web/Components/Pages/Editor.razor','src/PublisherStudio.Web/Components/Editor/InspectorPanel.razor','src/PublisherStudio.Web/Components/Editor/MediaStudio.razor']:
  req(rel,'mediaStudioInterop.js?v=2.8.8')
 ...
 print(f'PublisherStudio 2.8.8 strict architecture source audit passed: {checks} checks; explicit renderer helpers: {sum(len(v) for v in helpers.values())}.')
except Exception as e:
 print(f'PublisherStudio 2.8.8 source audit failed: {e}',file=sys.stderr); raise SystemExit(1)
