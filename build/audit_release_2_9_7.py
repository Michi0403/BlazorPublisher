#!/usr/bin/env python3
"""Source-only release audit for PublisherStudio 2.9.7."""
...
for rel in [
    ...
    text = read(rel)
    req(text, '<Version>2.9.7</Version>', f'2.9.7 version in {rel}')
...
if package.get('version') != '2.9.7' or lock.get('version') != '2.9.7' or lock.get('packages',{}).get('',{}).get('version') != '2.9.7':
    failures.append('npm package/package-lock version alignment is not 2.9.7')
else: checks.append('npm package/package-lock 2.9.7 alignment')
...
# 2.9.7 authoritative build-feedback compliance.
new_dialog = read('src/PublisherStudio.Web/Components/Editor/NewPublicationDialog.razor')
panel_library = read('src/PublisherStudio.Web/Components/Editor/PanelLibrary.razor')
for text, needle, label in [
    (new_dialog, '@inject IUserNotificationService Notifications', 'NewPublicationDialog notification service injection'),
    (new_dialog, 'Notifications.Error(_error, LT("New from template"), nameof(NewPublicationDialog));', 'NewPublicationDialog recoverable failure notification'),
    (panel_library, '@inject IUserNotificationService Notifications', 'PanelLibrary notification service injection'),
    (panel_library, 'Notifications.Error(_templateError, LT("Panel library"), nameof(PanelLibrary));', 'PanelLibrary recoverable failure notification'),
    (panel_studio, 'var divTemplateId = toolId["divtemplate:".Length..];', 'separate Div-template local identifier'),
    (panel_studio, 'out var componentTemplateId', 'separate component-template parsed identifier'),
    (panel_studio, 'Notifications.Warning(', 'Panel Studio recoverable Div-template warning'),
]: req(text, needle, label)
if 'var templateId = toolId["divtemplate:".Length..];' in panel_studio or 'out var templateId' in panel_studio[panel_studio.find('private PublicationElement? CreateToolElement'):panel_studio.find('private void BeginPaletteDrag')]:
    failures.append('PanelStudio CreateToolElement retains the CS0136 templateId collision pattern')
else:
    checks.append('PanelStudio CS0136 local-name collision removed')
project = read('src/PublisherStudio.Web/PublisherStudio.Web.csproj')
req(project, r'<Content Update="Configuration\**\*" CopyToOutputDirectory="PreserveNewest" CopyToPublishDirectory="PreserveNewest" />', 'template seed output/publish contract')

# Reproduce the new-operational-component branch of Assert-ComponentDiagnostics.ps1.
catch_count = len(re.findall(r'\bcatch\b', new_dialog))
log_count = len(re.findall(r'\.Log(?:Trace|Debug|Information|Warning|Error|Critical)\s*\(', new_dialog))
notification_count = len(re.findall(r'\b(?:Notifications|OperationalNotifications)\.(?:Success|Info|Warning|Error)\s*\(', new_dialog))
if catch_count < 1 or log_count < 1 or notification_count < 1:
    failures.append(f'NewPublicationDialog diagnostics boundary incomplete: catch={catch_count}, log={log_count}, notification={notification_count}')
else:
    checks.append('NewPublicationDialog catch/log/notification diagnostics boundary')
...
if failures:
    print('PublisherStudio 2.9.7 source release audit failed:')
    for failure in failures: print('  -', failure)
    raise SystemExit(1)
print(f'PublisherStudio 2.9.7 source release audit passed: {len(checks)} checks.')
