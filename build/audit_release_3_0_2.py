#!/usr/bin/env python3
"""Source-only release audit for PublisherStudio 3.0.2. Does not invoke dotnet/pwsh."""
...
if lock.get("version") == "3.0.2" and lock.get("packages", {}).get("", {}).get("version") == "3.0.2": PASS.append("package-lock top identity")
else: FAIL.append("package-lock top identity is not 3.0.2")

for rel in ["RELEASE.md", "CHANGELOG-v3.0.2-PYTHON-RUNTIME-RESOLUTION.md", "VALIDATION-v3.0.2-source.md"]:
    require(rel, "3.0.2", f"release documentation identity {rel}")
...
for asset in ["site.css", "localizationRuntime.js", "videoEffectRuntime.js", "componentRuntime.js", "publisherInterop.js"]:
    if f"{asset}?v=3.0.2" in app: PASS.append(f"cache identity {asset}")
    else: FAIL.append(f"cache identity missing for {asset}")
require("src/PublisherStudio.Web/Components/Editor/MediaStudio.razor", "mediaStudioInterop.js?v=3.0.2", "MediaStudio module identity")
require("src/PublisherStudio.Web/Components/Editor/InspectorPanel.razor", "mediaStudioInterop.js?v=3.0.2", "Inspector module identity")
...
if FAIL:
    print("PublisherStudio 3.0.2 static release audit failed:")
    for item in FAIL: print(" -", item)
    raise SystemExit(1)
print(f"PublisherStudio 3.0.2 static release audit passed: {len(PASS)} checks.")
