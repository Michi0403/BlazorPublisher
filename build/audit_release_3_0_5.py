#!/usr/bin/env python3
"""Source-only release audit for PublisherStudio 3.0.5. Does not invoke dotnet/pwsh."""
...
if lock.get("version") == "3.0.5" and lock.get("packages", {}).get("", {}).get("version") == "3.0.5": PASS.append("package-lock top identity")
else: FAIL.append("package-lock top identity is not 3.0.5")

for rel in ["RELEASE.md", "CHANGELOG-v3.0.5-NATIVE-DISCOVERY-RESOLUTION-HARDENING.md", "VALIDATION-v3.0.5-source.md"]:
    require(rel, "3.0.5", f"release documentation identity {rel}")
...
for asset in ["site.css", "localizationRuntime.js", "videoEffectRuntime.js", "componentRuntime.js", "publisherInterop.js"]:
    if f"{asset}?v=3.0.5" in app: PASS.append(f"cache identity {asset}")
    else: FAIL.append(f"cache identity missing for {asset}")
require("src/PublisherStudio.Web/Components/Editor/MediaStudio.razor", "mediaStudioInterop.js?v=3.0.5", "MediaStudio module identity")
require("src/PublisherStudio.Web/Components/Editor/InspectorPanel.razor", "mediaStudioInterop.js?v=3.0.5", "Inspector module identity")
...
# 3.0.5 native-discovery resolution hardening must stay independent of using-directive ordering.
native = read("src/PublisherStudio.Web/Services/Streaming/Capture/NativeDeviceDiscoveryPlatformServices.cs")
for marker in [
    "global::PublisherStudio.BusinessObjects.PublisherRuntimePattern.NativeDirectShowDevice",
    "global::PublisherStudio.BusinessObjects.PublisherRuntimePattern.NativeAvFoundationDevice",
]:
    if marker in native: PASS.append(f"fully qualified native runtime pattern {marker}")
    else: FAIL.append(f"fully qualified native runtime pattern missing: {marker}")
...
windows_guard = platform.find("if (OperatingSystem.IsWindows())")
unix_call = platform.find("File.SetUnixFileMode")
if windows_guard >= 0 and unix_call > windows_guard and "PlatformNotSupportedException" in platform[windows_guard:unix_call]:
    PASS.append("Unix file-mode analyzer guard")
else:
    FAIL.append("Unix file-mode analyzer guard missing or ordered after Unix API call")
...
if FAIL:
    print("PublisherStudio 3.0.5 static release audit failed:")
    for item in FAIL: print(" -", item)
    raise SystemExit(1)
print(f"PublisherStudio 3.0.5 static release audit passed: {len(PASS)} checks.")
