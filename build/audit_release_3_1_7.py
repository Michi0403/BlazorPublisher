...
major, minor, patch = (3, 1, 7)
if minor > 9 or patch > 9:
    errors.append("release version violates the one-digit minor/patch slot policy")
require("src/PublisherStudio.Web/package.json", '"version": "3.1.7"')
require("src/PublisherStudio.Web/package-lock.json", '"version": "3.1.7"')
require("RELEASE.md", "CHANGELOG-v3.1.7-HOST-AWARE-LOCAL-FIRST-RELEASE.md")
require("RELEASE.md", "VALIDATION-v3.1.7-source.md")
text("CHANGELOG-v3.1.7-HOST-AWARE-LOCAL-FIRST-RELEASE.md")
text("VALIDATION-v3.1.7-source.md")
require("docs/docfx.json", '"publisherstudioVersion": "3.1.7"')
require("docs/pdf/toc.yml", "PublisherStudio-3.1.7.pdf")
...
for marker in (
    ...
    "LocalGPT', 'NuGet'",
    "Publish-ReleasePackagingPackage.ps1",
    "No network download was attempted because no -PackageUrl was supplied.",
...
# Host-aware release orchestration, local-first helper resolution, and optional native finishing.
for marker in (
    '"all-rids"',
    ...
    if marker not in build:
        errors.append(f"Build-Release.ps1 missing host-aware marker: {marker}")
...
require("Build-AllRuntimes.ps1", "all maintained runtimes supported by this host")
require("Build-AllRuntimes.cmd", "chcp 65001 >nul")

for marker in (
    '[ValidateSet("Release", "Debug")][string]$Configuration = "Release"',
    "LocalGPT', 'src'",
    "Publish-ReleasePackagingPackage.ps1",
    "explicitly pass -ReleasePackagingPackageUrl",
    "Downloading authoritative LocalGPT release-packaging package $Version from the explicitly configured URL",
):
    if marker not in ensure:
        errors.append(f"Ensure-ReleasePackagingPackage.ps1 missing local-first marker: {marker}")
if "https://github.com/Michi0403/LocalGPT/releases/latest/download/$packageName" in ensure:
    errors.append("Ensure-ReleasePackagingPackage.ps1 still has an implicit GitHub download fallback")
...
if errors:
    print("PublisherStudio 3.1.7 static release audit FAILED:")
    for error in errors:
        print(" -", error)
    raise SystemExit(1)
print("PublisherStudio 3.1.7 static release audit passed.")
