...
for rel in ('src/PublisherStudio.Web/PublisherStudio.Web.csproj','src/PublisherStudio.InstallerConsole/PublisherStudio.InstallerConsole.csproj'):
    req(rel, '<Version>3.2.0</Version>')
req('src/PublisherStudio.Web/package.json', '"version": "3.2.0"')
req('src/PublisherStudio.Web/package-lock.json', '"version": "3.2.0"')
req('docs/docfx.json', '"publisherstudioVersion": "3.2.0"')
req('docs/pdf/toc.yml', 'PublisherStudio-3.2.0.pdf')
req('RELEASE.md', 'CHANGELOG-v3.2.0-WINDOWS-WSL-LINUX-RELEASE.md')
req('RELEASE.md', 'VALIDATION-v3.2.0-source.md')
text('CHANGELOG-v3.2.0-WINDOWS-WSL-LINUX-RELEASE.md')
text('VALIDATION-v3.2.0-source.md')
...
if any(x > 9 for x in (2, 0)):
    errors.append('version violates one-digit minor/patch policy')
if errors:
    print('PublisherStudio 3.2.0 static release audit FAILED:')
    for error in errors:
        print(' -', error)
    raise SystemExit(1)
print('PublisherStudio 3.2.0 static release audit passed.')
