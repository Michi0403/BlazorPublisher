...
for rel in ('src/PublisherStudio.Web/package.json','src/PublisherStudio.Web/package-lock.json'):
    try:
        ...
        if data.get('version')!='3.2.8': errors.append(f'{rel} top-level version is not 3.2.8')
        if rel.endswith('package-lock.json') and data.get('packages',{}).get('',{}).get('version')!='3.2.8':
            errors.append('package-lock.json root package version is not 3.2.8')
    except Exception as exc: errors.append(f'{rel} JSON parse failed: {exc}')
try:
    data=json.loads(text('docs/docfx.json'))
    if data.get('build',{}).get('globalMetadata',{}).get('publisherstudioVersion')!='3.2.8': errors.append('docs/docfx.json publisherstudioVersion is not 3.2.8')
except Exception as exc: errors.append(f'docs/docfx.json JSON parse failed: {exc}')
req('docs/pdf/toc.yml','PublisherStudio-3.2.8.pdf')
req('docs/index.md','**Version 3.2.8**')
req('RELEASE.md','# PublisherStudio 3.2.8')
req('CHANGELOG-v3.2.8-DOCFX-PDF-FALLBACK-REPAIR.md','Version advanced from 3.2.7 to 3.2.8')
req('VALIDATION-v3.2.8-source.md','# PublisherStudio 3.2.8 source validation')
...
if errors:
    print('PublisherStudio 3.2.8 static release audit FAILED:')
    for e in errors: print(' -',e)
    raise SystemExit(1)
print('PublisherStudio 3.2.8 source audit passed.')
