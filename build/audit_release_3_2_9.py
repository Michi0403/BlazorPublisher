...
for rel in ('src/PublisherStudio.Web/package.json','src/PublisherStudio.Web/package-lock.json'):
    try:
        ...
        if data.get('version')!='3.2.9': errors.append(f'{rel} top-level version is not 3.2.9')
        if rel.endswith('package-lock.json') and data.get('packages',{}).get('',{}).get('version')!='3.2.9':
            errors.append('package-lock.json root package version is not 3.2.9')
    except Exception as exc: errors.append(f'{rel} JSON parse failed: {exc}')
try:
    data=json.loads(text('docs/docfx.json'))
    if data.get('build',{}).get('globalMetadata',{}).get('publisherstudioVersion')!='3.2.9': errors.append('docs/docfx.json publisherstudioVersion is not 3.2.9')
except Exception as exc: errors.append(f'docs/docfx.json JSON parse failed: {exc}')
req('docs/pdf/toc.yml','PublisherStudio-3.2.9.pdf')
req('docs/index.md','**Version 3.2.9**')
req('RELEASE.md','# PublisherStudio 3.2.9')
req('CHANGELOG-v3.2.9-MACOS-GATEKEEPER-NOTARIZATION.md','Version advanced from 3.2.8 to 3.2.9')
req('VALIDATION-v3.2.9-source.md','# PublisherStudio 3.2.9 source validation')
...
docbuild=text('build/Build-Documentation.ps1')
for marker in (
    '$maximumBrowserPrintSourcePages = 600',
    'function Ensure-PublisherStudioDocfxToolForPdfFallback',
    'PDF fallback requires DocFX; resolving the repository-local tool now.',
    ...
    'above the host browser-print limit of $maximumBrowserPrintSourcePages; using the DocFX PDF plug-in directly.',
):
    if marker not in docbuild: errors.append(f'build/Build-Documentation.ps1 missing: {marker}')

if errors:
    print('PublisherStudio 3.2.9 static release audit FAILED:')
    for e in errors: print(' -',e)
    raise SystemExit(1)
print('PublisherStudio 3.2.9 source audit passed.')
