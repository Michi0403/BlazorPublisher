...
for rel in ('src/PublisherStudio.Web/package.json','src/PublisherStudio.Web/package-lock.json'):
    try:
        ...
        if data.get('version')!='3.3.6': errors.append(f'{rel} top-level version is not 3.3.6')
        if rel.endswith('package-lock.json') and data.get('packages',{}).get('',{}).get('version')!='3.3.6': errors.append('package-lock.json root package version is not 3.3.6')
    except Exception as exc: errors.append(f'{rel} JSON parse failed: {exc}')
try:
    data=json.loads(text('docs/docfx.json'))
    if data.get('build',{}).get('globalMetadata',{}).get('publisherstudioVersion')!='3.3.6': errors.append('docs/docfx.json publisherstudioVersion is not 3.3.6')
except Exception as exc: errors.append(f'docs/docfx.json JSON parse failed: {exc}')
req('docs/pdf/toc.yml','PublisherStudio-3.3.6.pdf')
req('docs/index.md','**Version 3.3.6**')
req('RELEASE.md','# PublisherStudio 3.3.6')
req('CHANGELOG-v3.3.6-ADAPTIVE-MANAGED-PDF-PIPELINE.md','LocalGPT.ReleasePackaging')
req('VALIDATION-v3.3.6-source.md','# PublisherStudio 3.3.6 source validation')
...
# Adaptive managed PDF pipeline and self-contained helper source fallback.
...
for marker in ('case "pdf-merge"','case "pdf-optimize"','PdfReader.Open','PdfDocumentOpenMode.Import','--optimize-images'):
    if marker not in text('src/LocalGPT.ReleasePackaging/Program.cs'): errors.append(f'vendored packaging helper missing PDF marker: {marker}')
...
for marker in ('src', 'LocalGPT.ReleasePackaging', 'dotnet pack', 'https://api.nuget.org/v3/index.json'):
    if marker not in ensure: errors.append(f'build/Ensure-ReleasePackagingPackage.ps1 missing self-contained helper marker: {marker}')
...
if errors:
    print('PublisherStudio 3.3.6 static release audit FAILED:')
    for e in errors: print(' -',e)
    raise SystemExit(1)
print('PublisherStudio 3.3.6 source audit passed.')
