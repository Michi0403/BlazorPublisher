...
for rel in ('src/PublisherStudio.Web/package.json','src/PublisherStudio.Web/package-lock.json'):
    try:
        ...
        if data.get('version')!='3.3.9': errors.append(f'{rel} top-level version is not 3.3.9')
        if rel.endswith('package-lock.json') and data.get('packages',{}).get('',{}).get('version')!='3.3.9': errors.append('package-lock.json root package version is not 3.3.9')
    except Exception as exc: errors.append(f'{rel} JSON parse failed: {exc}')
try:
    data=json.loads(text('docs/docfx.json'))
    if data.get('build',{}).get('globalMetadata',{}).get('publisherstudioVersion')!='3.3.9': errors.append('docs/docfx.json publisherstudioVersion is not 3.3.9')
except Exception as exc: errors.append(f'docs/docfx.json JSON parse failed: {exc}')
req('docs/pdf/toc.yml','PublisherStudio-3.3.9.pdf')
req('docs/index.md','**Version 3.3.9**')
req('RELEASE.md','# PublisherStudio 3.3.9')
req('CHANGELOG-v3.3.9-NOTARY-KEYCHAIN-REGRESSION-REPAIR.md','html-browser-chunked')
req('VALIDATION-v3.3.9-source.md','# PublisherStudio 3.3.9 source validation')
...
# 3.3.9 regression: PublisherStudio generates the same adaptive chunked browser PDF mode and
...
if errors:
    print('PublisherStudio 3.3.9 static release audit FAILED:')
    for e in errors: print(' -',e)
    raise SystemExit(1)
print('PublisherStudio 3.3.9 source audit passed.')
