...
for rel in ('src/PublisherStudio.Web/package.json','src/PublisherStudio.Web/package-lock.json'):
    try:
        ...
        if d.get('version')!='3.4.2': errors.append(f'{rel} version != 3.4.2')
...
try:
    ...
    if meta.get('publisherstudioVersion')!='3.4.2': errors.append('docs/docfx.json publisherstudioVersion != 3.4.2')
except Exception as exc: errors.append(f'docfx json parse failed: {exc}')
for rel,mark in (
    ('docs/index.md','**Version 3.4.2**'),('docs/pdf/toc.yml','PublisherStudio-3.4.2.pdf'),('RELEASE.md','# PublisherStudio 3.4.2'),
    ('CHANGELOG-v3.4.2-NOTARY-ORCHESTRATION-RECOVERY.md','Notary orchestration recovery'),('VALIDATION-v3.4.2-source.md','# PublisherStudio 3.4.2 source validation')): req(rel,mark)
...
if errors:
    print('PublisherStudio 3.4.2 static release audit FAILED:')
    for e in errors: print(' -',e)
    sys.exit(1)
print('PublisherStudio 3.4.2 source audit passed.')
