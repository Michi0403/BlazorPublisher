...
for rel in ('src/PublisherStudio.Web/package.json','src/PublisherStudio.Web/package-lock.json'):
    try:
        ...
        if d.get('version')!='3.4.4': errors.append(f'{rel} version != 3.4.4')
...
try:
    ...
    if meta.get('publisherstudioVersion')!='3.4.4': errors.append('docs/docfx.json publisherstudioVersion != 3.4.4')
except Exception as exc: errors.append(f'docfx json parse failed: {exc}')
for rel,mark in (
    ('docs/index.md','**Version 3.4.4**'),('docs/pdf/toc.yml','PublisherStudio-3.4.4.pdf'),('RELEASE.md','# PublisherStudio 3.4.4'),
    ('CHANGELOG-v3.4.4-ARTIFACT-LOCAL-NOTARY-TRANSACTIONS.md','artifact-local notarization transactions'),('VALIDATION-v3.4.4-source.md','# PublisherStudio 3.4.4 source validation')): req(rel,mark)
...
if errors:
    print('PublisherStudio 3.4.4 static release audit FAILED:')
    for e in errors: print(' -',e)
    sys.exit(1)
print('PublisherStudio 3.4.4 source audit passed.')
