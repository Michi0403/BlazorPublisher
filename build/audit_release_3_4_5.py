...
for rel in ('src/PublisherStudio.Web/package.json','src/PublisherStudio.Web/package-lock.json'):
    try:
        ...
        if d.get('version')!='3.4.5': errors.append(f'{rel} version != 3.4.5')
...
try:
    ...
    if meta.get('publisherstudioVersion')!='3.4.5': errors.append('docs/docfx.json publisherstudioVersion != 3.4.5')
except Exception as exc: errors.append(f'docfx json parse failed: {exc}')
for rel,mark in (
    ('docs/index.md','**Version 3.4.5**'),('docs/pdf/toc.yml','PublisherStudio-3.4.5.pdf'),('RELEASE.md','# PublisherStudio 3.4.5'),
    ('CHANGELOG-v3.4.5-PUBLISHER-DOCUMENTATION-PARITY-REPAIR.md','documentation parity repair'),('VALIDATION-v3.4.5-source.md','# PublisherStudio 3.4.5 source validation')): req(rel,mark)
...
if errors:
    print('PublisherStudio 3.4.5 static release audit FAILED:')
    for e in errors: print(' -',e)
    sys.exit(1)
print('PublisherStudio 3.4.5 source audit passed.')
