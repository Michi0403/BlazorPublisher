#!/usr/bin/env python3
"""Source-only release checks for PublisherStudio 3.5.3."""
...
for rel, needle in (
    ...
    ("CHANGELOG-v3.5.3-FRONTEND-RACE-GALLERY-PKG-REPAIR.md", "# PublisherStudio 3.5.3"),
    ("VALIDATION-v3.5.3-source.md", "# PublisherStudio 3.5.3 source validation"),
):
    require(rel, needle)
...
# 3.5.3 frontend asset/runtime synchronization and race repair.
...
for rel in (
    ...
    text = read(rel)
    if '"./js/publisherInterop.js?v=3.5.3"' not in text:
...
# 3.5.3 release parser and source identity prevent stale artifacts.
...
if ERRORS:
    print("PublisherStudio 3.5.3 source audit FAILED:")
    for error in ERRORS:
        print(f" - {error}")
    sys.exit(1)
print(
