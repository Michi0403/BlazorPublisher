#!/usr/bin/env python3
"""Source-only release checks for PublisherStudio 3.5.4."""
...
for rel, needle in (
    ...
    ("CHANGELOG-v3.5.4-WINDOWS-UPDATER-PROCESS-OWNERSHIP.md", "# PublisherStudio 3.5.4"),
    ("VALIDATION-v3.5.4-source.md", "# PublisherStudio 3.5.4 source validation"),
):
    require(rel, needle)
...
# 3.5.4 frontend asset/runtime synchronization and race repair.
...
for rel in (
    ...
    text = read(rel)
    if '"./js/publisherInterop.js?v=3.5.4"' not in text:
...
# 3.5.4 Windows updater must stop only the installed runtime before extraction.
...
# 3.5.4 release parser and source identity prevent stale artifacts.
...
if ERRORS:
    print("PublisherStudio 3.5.4 source audit FAILED:")
    for error in ERRORS:
        print(f" - {error}")
    sys.exit(1)
print(
