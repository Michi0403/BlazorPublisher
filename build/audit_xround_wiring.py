...
print(
    ...
    'human/transition/depth gates are present; and each direct live user message can immediately restart only the ordered foreground participant while remaining available to later Council heartbeat context; live lanes pre-register every provider-qualified participant; and the Chat UI exposes per-host plus per-road concurrency controls.'
)
