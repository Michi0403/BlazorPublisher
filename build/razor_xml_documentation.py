...
def component_summary(path: Path) -> str:
    ...
    label = xd.words(name)
    if 'Pages' in parts:
        return f'Renders the {label} PublisherStudio Razor page and coordinates the page-local state, commands, and user interactions exposed through its markup.'
    if 'Layout' in parts:
        return f'Renders the {label} PublisherStudio layout component and coordinates shell, navigation, or shared presentation behavior used by the application UI.'
    if 'Shared' in parts:
        return f'Renders the reusable {label} Razor component and exposes its parameter-driven UI behavior to PublisherStudio pages and layout surfaces.'
    if name == 'App':
        return 'Defines the root PublisherStudio Razor application component, establishing the application shell and top-level component composition used at startup.'
    if name == 'Routes':
        return 'Defines the PublisherStudio Razor routing component, selecting routable pages and shared route behavior for interactive server navigation.'
    return f'Renders the {label} Razor component and coordinates the component-local state and presentation behavior used by the surrounding PublisherStudio interface.'
