...
/// <summary>
/// Persists PublisherStudio setup diagnostics to a durable per-user transcript so installer and startup failures remain inspectable after the console closes.
/// </summary>
internal sealed class SetupFileLoggerProvider : ILoggerProvider
{
    ...
    public SetupFileLoggerProvider(string logPath)
    {
        ...
        File.AppendAllText(
            this.logPath,
            $"{Environment.NewLine}===== PublisherStudio setup session {DateTimeOffset.UtcNow:O} ====={Environment.NewLine}",
            Encoding.UTF8);
    }
