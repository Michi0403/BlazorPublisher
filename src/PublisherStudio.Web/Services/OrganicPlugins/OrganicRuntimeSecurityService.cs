    ...
    ILogger<OrganicRuntimeSecurityService> logger) : IOrganicRuntimeSecurityService
{
    ...
    public async Task<OneWireRuntimeSecurityStatus> GetStatusAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogError(ex, "Could not read the PublisherStudio runtime 1-Wire security status.");
    ...
    public async Task EnsureCreatedAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogError(ex, "Could not create the PublisherStudio runtime 1-Wire secret.");
            throw;
        }
        finally { gate.Release(); }
    }
    ...
    public async Task RegenerateAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            cached = replacement;
            logger.LogWarning("Regenerated the PublisherStudio runtime 1-Wire secret. All previously trusted peers are intentionally invalidated.");
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException or CryptographicException or JsonException)
        {
            logger.LogError(ex, "Could not regenerate the PublisherStudio runtime 1-Wire secret.");
            throw;
        }
        finally { gate.Release(); }
    }
    ...
    public async Task DeleteAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            cached = null;
            logger.LogWarning("Deleted the PublisherStudio runtime 1-Wire secret. The local identity and every trusted peer binding were reset.");
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            logger.LogError(ex, "Could not delete the PublisherStudio runtime 1-Wire secret.");
            throw;
        }
        finally { gate.Release(); }
    }
    ...
    public async Task<OneWireSecurityDescriptor> GetPublicDescriptorAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogWarning(ex, "Could not expose the PublisherStudio public 1-Wire security descriptor.");
    ...
    public async Task<OneWirePairingTicket> CreatePairingTicketAsync(TimeSpan lifetime, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                PeerId = $"publisherstudio:{Environment.MachineName}",
                DisplayName = "PublisherStudio",
            ...
            logger.LogInformation("Created a PublisherStudio 1-Wire public pairing ticket valid until {ExpiresUtc}.", ticket.ExpiresUtc);
            return ticket;
        }
        ...
        {
            logger.LogError(ex, "Could not create a PublisherStudio 1-Wire pairing ticket.");
            throw;
        }
        finally { gate.Release(); }
    }
    ...
    public async Task<string> GetOtpAuthUriAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var file = await LoadCoreAsync(createWhenMissing: true, cancellationToken).ConfigureAwait(false)
                ?? throw new InvalidOperationException("The runtime security file could not be created.");
            var label = Uri.EscapeDataString($"PublisherStudio:{Environment.MachineName}");
            var issuer = Uri.EscapeDataString("PublisherStudio 1-Wire");
        ...
        {
            logger.LogError(ex, "Could not create the PublisherStudio 1-Wire Authenticator URI.");
            throw;
        }
        finally { gate.Release(); }
    }
    ...
    public async Task<bool> EstablishTrustAsync(OneWireTrustEstablishmentRequest request, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                logger.LogWarning("Rejected a PublisherStudio 1-Wire trust request for peer {PeerId}: the local MFA code was invalid.", request.Ticket.PeerId);
                return false;
            }
            ...
            cached = file;
            logger.LogInformation("Established MFA-verified PublisherStudio 1-Wire trust for peer {PeerId} until {ValidUntilUtc}.", trusted.PeerId, trusted.ValidUntilUtc);
            return true;
        }
        ...
        {
            logger.LogError(ex, "Could not establish PublisherStudio 1-Wire trust for peer {PeerId}.", request.Ticket?.PeerId);
            throw;
        }
        finally { gate.Release(); }
    }
    ...
    public async Task<bool> RevokeTrustAsync(string peerId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (removed)
            {
                await PersistCoreAsync(file, cancellationToken).ConfigureAwait(false);
                cached = file;
                logger.LogWarning("Revoked PublisherStudio 1-Wire trust for peer {PeerId}.", peerId);
            }
            return removed;
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException or JsonException or CryptographicException)
        {
            logger.LogError(ex, "Could not revoke PublisherStudio 1-Wire trust for peer {PeerId}.", peerId);
            throw;
        }
        finally { gate.Release(); }
    }
    ...
    public async Task<IReadOnlyList<OneWireTrustedPeerDescriptor>> GetTrustedPeersAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogWarning(ex, "Could not read PublisherStudio trusted 1-Wire peers.");
            return [];
        }
        finally { gate.Release(); }
    }
    ...
    public async Task ProtectOutgoingAsync(OrganicWireEnvelope envelope, CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogError(ex, "Could not protect outgoing PublisherStudio 1-Wire message {MessageId} for {PeerId}.", envelope.MessageId, envelope.TargetPeerId);
            throw;
        }
        finally { gate.Release(); }
    }
    ...
    public async Task UnprotectIncomingAsync(OrganicWireEnvelope envelope, CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogError(ex, "Could not verify or decrypt incoming PublisherStudio 1-Wire message {MessageId} from {PeerId}.", envelope.MessageId, envelope.SourcePeerId);
            throw;
        }
        finally { gate.Release(); }
    }
    ...
    try
    {
            ...
            if (!File.Exists(path))
            {
                ...
                cached = created;
                logger.LogInformation("Created the PublisherStudio runtime 1-Wire secret at {SecretPath}.", path);
                return created;
            }
            var json = await File.ReadAllTextAsync(path, cancellationToken).ConfigureAwait(false);
            var file = JsonSerializer.Deserialize<RuntimeSecretFile>(json, jsonOptions)
                ?? throw new JsonException("The PublisherStudio 1-Wire secret file is empty.");
            ValidateSecret(file);
            cached = file;
            return file;
    ...
    catch (Exception __serviceMethodException)
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(LoadCoreAsync)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(LoadCoreAsync)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(PersistCoreAsync)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(PersistCoreAsync)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(ResolveSecretPath)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(ResolveSecretPath)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(CanWriteDirectory)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(CanWriteDirectory)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(CreateSecret)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(CreateSecret)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(ValidateSecret)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(ValidateSecret)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(CreateStatus)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(CreateStatus)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(CreatePublicDescriptor)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(CreatePublicDescriptor)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(ValidatePairingTicket)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(ValidatePairingTicket)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(BuildTicketBytes)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(BuildTicketBytes)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(BuildSignatureBytes)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(BuildSignatureBytes)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(BuildAssociatedData)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(BuildAssociatedData)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(DerivePeerKey)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(DerivePeerKey)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(HkdfSha256)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(HkdfSha256)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(VerifyTotp)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(VerifyTotp)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(Base32Encode)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(Base32Encode)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(IsCurrentTrust)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(IsCurrentTrust)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(IsSecurityBootstrap)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(IsSecurityBootstrap)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(Clamp)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(Clamp)} failed.");
        throw;
    }
}
    ...
    {
        if (__serviceMethodException is OperationCanceledException)
            logger.LogDebug(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(CloneTrustedPeer)} was canceled.");
        else
            logger.LogError(__serviceMethodException, $"Service method {nameof(OrganicRuntimeSecurityService)}.{nameof(CloneTrustedPeer)} failed.");
        throw;
    }
}
